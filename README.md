...
- `NEYNAR_API_KEY` — required (used to fetch `experimental.neynar_user_score` on API calls).  
- `UPSTASH_REDIS_REST_URL` — required (snapshot storage).
- `UPSTASH_REDIS_REST_TOKEN` — required (snapshot storage).

Optional:
- `BASE_RPC_URL` — not required for score fetch; only relevant if you extend onchain verification.
...
> Note: Farcaster Mini App chrome (no address bar) only appears when opened inside a Farcaster client. Local dev will run in a normal browser.

---

## Farcaster manifest

This repo serves a manifest at:

`https://neynar-history.vercel.app/.well-known/farcaster.json`

It contains the required `miniapp` fields. For full ownership verification + discovery, add the signed `accountAssociation` section using Farcaster’s manifest tool as described in the Farcaster/Neynar docs.
