...
function HomeView({ ctx }: { ctx: any }) {
  ...
  const isConnected = Boolean(auth?.fid || ctx?.user?.fid);
      ...
      <div className="card">
        <div className="header">
          ...
          <div className="row" style={{ gap: 10 }}>
            ...
            <button
              className={"btn primary" + (isConnected ? " ghost" : "")}
              onClick={isConnected ? undefined : handleSignIn}
              disabled={loading || isConnected}
              title={isConnected ? "Already connected" : undefined}
            >
              {isConnected ? "Connected" : (loading ? "Loading…" : "Sign in with Farcaster")}
            </button>
          </div>
        </div>
        ...
        <div className="section">
          <div className="row" style={{ alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div className="row" style={{ alignItems: "center" }}>
              ...
              <Avatar
                url={display?.pfp_url}
                handle={display?.username || display?.display_name || null}
                size={36}
              />
            </div>
          </div>
          ...
          <div className="card" style={{ boxShadow: "none", background: "var(--tint2)" }}>
            <div className="section">
              <div className="sub">Your latest known Neynar score</div>
              <div className="h2" style={{ marginTop: 6 }}>
                {me?.score != null ? me.score.toFixed(3) : "—"}
              </div>
          ...
          <div className="row" style={{ justifyContent: "flex-start" }}>
            <button
              className="btn"
              onClick={() => display?.fid && navigate({ name: "user", fid: Number(display.fid) })}
              disabled={!display?.fid}
            >
              View 90-day history
            </button>
          </div>
          ...
          <div className="sub" style={{ marginTop: 8 }}>Examples: @dwr, 3</div>
