...
function badgeFromDelta(delta: number) {
  ...
  return {
    label: 'Flat',
    cls: 'bg-slate-100 border-slate-200 text-slate-700 dark:bg-white/8 dark:border-white/15 dark:text-white/85',
  }
}
...
export function UserScoreView({ fid, onBack }: { fid: number; onBack: () => void }) {
  ...
  async function refreshNow() {
    setFreshening(true)
    try {
      const snap = await fetchOnchainScore(fid)
      if (!snap) {
        toast.push({ kind: 'error', title: 'Score unavailable right now. Try again in a moment.' })
        return
      }
      upsertSnapshot(snap)
      setSnaps((prev) => {
      ...
      // /api/score already stores a snapshot server-side. Pull latest snapshots back in.
      const server = await fetchServerSnapshots(fid, 90)
      setServerEnabled(server.enabled)
      if (server.snapshots.length) {
        setSnaps((prev) => {
          const merged = [...prev, ...server.snapshots].sort((a, b) => a.captured_at.localeCompare(b.captured_at))
          const uniq: Snapshot[] = []
          const seen = new Set<string>()
          for (const s of merged) {
            const k = `${s.captured_at}|${s.score}|${s.source}`
    ...
    <div className="space-y-5">
      <header className="flex flex-wrap items-center gap-3">
        ...
        {fid === me?.fid && me?.pfpUrl ? (
          <img
            src={me.pfpUrl}
            alt=""
            className="h-10 w-10 rounded-2xl border border-slate-200 object-cover dark:border-white/15"
            referrerPolicy="no-referrer"
          />
        ) : (
          <div className="h-10 w-10 rounded-2xl border border-slate-200 bg-slate-100 dark:border-white/15 dark:bg-white/10" />
        )}
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-slate-500 dark:text-white/65">FID {fid}</div>
        </div>
      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="text-xs text-slate-500 dark:text-white/65">Current score (0–1)</div>
            {loading ? (
              <Skeleton className="mt-2 h-10 w-40" />
            ) : (
              <div className="mt-2 text-4xl font-semibold tabular-nums">{latest ? latest.score.toFixed(3) : '—'}</div>
            )}
            <div className="mt-2 text-xs text-slate-500 dark:text-white/65">
          ...
          {[7, 30, 90].map((d) => (
            <button
              ...
              className={
                ...
                (range === d ? 'border-slate-300 bg-slate-100 dark:border-white/25 dark:bg-white/12' : 'border-slate-200 bg-white hover:bg-slate-50 dark:border-white/12 dark:bg-white/6 dark:hover:bg-white/8')
              }
              onClick={async () => {
        ...
        <div className="mt-5 h-64 w-full overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 p-2 dark:border-white/12 dark:bg-white/5">
          {loading ? (
            ...
            <div className="grid h-full place-items-center text-sm text-slate-500 dark:text-white/65">
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-slate-500 dark:text-white/60">Timestamp • score • change</div>
        </div>
        ...
        {loading ? (
          ...
          <div className="mt-4 overflow-hidden rounded-3xl border border-slate-200 dark:border-white/12">
            <table className="w-full text-left text-sm">
              <thead className="bg-white/6 text-xs text-slate-500 dark:text-white/70">
                <tr>
              ...
              <tbody>
                {rows
                  ...
                  .slice(0, 120)
                  .map((r) => (
                    <tr key={r.captured_at} className="border-t border-slate-200 dark:border-white/10">
          ...
          <div className="mt-4 text-sm text-slate-500 dark:text-white/65">No rows yet.</div>
        )}
      </Card>
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-slate-500 dark:text-white/60">Only points where score changed</div>
        </div>
        ...
        {loading ? (
          ...
          <div className="mt-4 space-y-2">
            {timeline
              ...
              .map((s, i) => {
                ...
                return (
                  <div key={s.captured_at} className="flex items-center gap-3 rounded-3xl border border-slate-200 dark:border-white/12 bg-white/6 p-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-500 dark:text-white/60">{formatDate(s.captured_at)}</div>
                    </div>
                    <div className="ml-auto text-xs text-slate-500 dark:text-white/70 tabular-nums">
          ...
          <div className="mt-4 text-sm text-slate-500 dark:text-white/65">
            No changes in this range yet.
          </div>
        )}

        <div className="mt-4 rounded-3xl border border-slate-200 dark:border-white/12 bg-white/8 p-4 text-xs text-slate-600 dark:text-white/70">
