...
import { fetchOnchainScore } from '../lib/score'
...
export function HomeView({ onOpenUser }: { onOpenUser: (fid: number) => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setSelfLoading(true)
      try {
        const snap = await fetchOnchainScore(identity.fid)
        if (snap) {
          upsertSnapshot(snap)
  ...
  async function onSignIn() {
    ...
    setQError(undefined)
    try {
      // IMPORTANT: call sign-in immediately from the click handler.
      // Awaiting *anything* first can break the host's "user gesture" requirement
      // and the request will silently never open (button spins forever).
      void haptic('selection')
      const { identity: id } = await signInWithFarcaster()
      if (!id.fid) throw new Error('Missing FID from host context.')

      // Fill in username/pfp from Neynar (more reliable than host context).
      try {
        const r = await fetch(`/api/score?fid=${encodeURIComponent(String(id.fid))}`)
        if (r.ok) {
          const j: any = await r.json()
          const u = j?.user
          if (u) {
            id.username = u.username ?? id.username
            id.displayName = u.display_name ?? u.displayName ?? id.displayName
            id.pfpUrl = u.pfp_url ?? u.pfpUrl ?? id.pfpUrl
          }
        }
      } catch {
        // ignore
