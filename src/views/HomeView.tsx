...
export function HomeView({ onOpenUser }: { onOpenUser: (fid: number) => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setSelfLoading(true)
      try {
        const snap = await fetchOnchainScore(identity.fid, 'base')
        if (snap) {
          upsertSnapshot(snap)
      ...
      <Card className="p-5">
        {!identity ? (
          <div className="flex flex-col gap-4">
            <div className="text-sm text-white/70 dark:text-white/70 text-gray-600">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-4">
              {identity.pfpUrl ? (
                ...
                <div className="h-11 w-11 rounded-2xl border border-white/15 bg-white/10" />
              )}
              <div className="min-w-0">
                <div className="truncate text-base font-semibold">
                  {identity.username ? '@' + identity.username : 'FID ' + identity.fid}
                </div>
                <div className="mt-0.5 text-xs text-white/65 dark:text-white/65 text-gray-500">
                  FID {identity.fid}
                </div>
              </div>
            ...
            <div className="rounded-3xl border border-white/12 bg-white/8 p-4">
              <div className="text-xs text-white/65 dark:text-white/65 text-gray-500">Your current score</div>
              {selfLoading ? (
                ...
                <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
                  ...
                  <div className="text-xs text-white/65 dark:text-white/65 text-gray-500">
