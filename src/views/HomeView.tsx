...
export function HomeView({ onOpenUser }: { onOpenUser: (fid: number) => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setSelfLoading(true)
      try {
        const r = await fetchServerScore(identity.fid, identity.fid)
        if (r.ok) {
          // Refresh identity from Neynar (fixes missing username/pfp from context on some clients)
          if (r.user?.username || r.user?.pfpUrl || r.user?.displayName) {
            const nextId: UserIdentity = {
              fid: identity.fid,
              username: r.user.username ?? identity.username,
              displayName: r.user.displayName ?? identity.displayName,
              pfpUrl: r.user.pfpUrl ?? identity.pfpUrl,
            }
            saveIdentity(nextId)
            setIdentity(nextId)
          }

          const snap: Snapshot = { fid: identity.fid, score: r.score, captured_at: r.fetched_at, source: 'api' }
          upsertSnapshot(snap)
          setSelfScore(snap.score)
          setSelfFetchedAt(snap.captured_at)
          await postServerSnapshot(snap)
          return
        }

        // Fallback to last local snapshot
        if (selfLastSnap) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-5">
        {!identity ? (
          <div className="flex flex-col gap-4">
            <div className="text-sm text-slate-600 dark:text-slate-600 dark:text-white/70">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-4">
              {identity.pfpUrl ? (
                <img
                  src={identity.pfpUrl}
                  alt=""
                  className="h-11 w-11 rounded-2xl border border-slate-200 object-cover dark:border-slate-200 dark:border-white/15"
                  referrerPolicy="no-referrer"
                />
              ) : (
                <div className="h-11 w-11 rounded-2xl border border-slate-200 bg-slate-100 dark:border-slate-200 dark:border-white/15 dark:bg-white dark:bg-white/10" />
              )}
              <div className="min-w-0">
                <div className="truncate text-base font-semibold">
                  {identity.username ? '@' + identity.username : 'FID ' + identity.fid}
                </div>
                <div className="mt-0.5 text-xs text-slate-500 dark:text-slate-600 dark:text-white/65">
                  FID {identity.fid}
                </div>
              </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-200 dark:border-white/12 dark:bg-slate-50 dark:bg-white/8">
              <div className="text-xs text-slate-500 dark:text-slate-600 dark:text-white/65">Your current score</div>
              {selfLoading ? (
                ...
                <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-600 dark:text-white/65">
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 rounded-3xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-200 dark:border-white/12 dark:bg-slate-50 dark:bg-white/8 text-xs text-slate-600 dark:text-slate-600 dark:text-white/70">
